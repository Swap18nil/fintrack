...
export const Sidebar = () => {
    ...
    return (
        <>
            <aside className={`shrink-0 bg-white border-r-2 border-gray-200 flex flex-col transition-all duration-300 ${isSidebarOpen ? 'w-64' : 'w-22'}`}>
                <div className="h-16 flex items-center px-6 "
                    onClick={() => handleCollapse()}>
                    <Wallet className="w-6 h-6 text-blue-600 mr-2" />
                    {
                        (isSidebarOpen && <span className="text-xl font-bold text-gray-800">FinTrack</span>)
                    }
                ...
                <div className="p-4 border-t-2 border-gray-200">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2 truncate">
                            ...
                            {
                                isSidebarOpen && <span className="text-sm font-medium text-gray-700 truncate max-w-[120px]">
                                    {user?.email}
                                </span>
                            }
